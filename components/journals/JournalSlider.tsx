...
function clarivateLabel(status: VerificationResult["indexing"]["clarivate"]) {
  if (status === "configured") return "بيانات الاعتماد موجودة؛ المحول المرخّص غير مفعّل";
  if (status === "unavailable") return "الخدمة غير متاحة";
  return "لم يُفحص عبر Clarivate";
}
...
export default function JournalSlider() {
  ...
  async function runVerification(inputIssn: string, inputTitle: string) {
    const cleanIssn = inputIssn.trim();
    const cleanTitle = inputTitle.trim();
    if (!cleanIssn && !cleanTitle) return;
    ...
    try {
      ...
      if (cleanIssn) params.set("issn", cleanIssn);
      ...
      setError(
        cause instanceof Error ? cause.message : "فشل التحقق من المجلة."
      );
    } finally {
      setLoading(false);
    }
  ...
  useEffect(() => {
    ...
    const initialIssn = cleanParam(params.get("issn"));
    const initialTitle = cleanParam(params.get("title"));

    if (!initialIssn && !initialTitle) return;

    setIssn(initialIssn);
    setTitle(initialTitle);
    void runVerification(initialIssn, initialTitle);
  }, []);

  async function verify(event: FormEvent<HTMLFormElement>) {
    event.preventDefault();
    await runVerification(issn, title);
  }

  return (
    <main
      className="min-h-screen bg-slate-50 px-3 py-4 sm:px-6 sm:py-8"
      dir="rtl"
    >
      <section className="mx-auto max-w-6xl">
        <header className="mb-5 flex flex-col gap-4 rounded-[28px] border border-slate-200 bg-white p-5 shadow-sm sm:p-7 lg:flex-row lg:items-center lg:justify-between">
          <div>
            ...
            <h1 className="mt-2 text-3xl font-black text-slate-950">
              التحقق من المجلات والتصنيف
            </h1>
            <p className="mt-3 max-w-3xl text-sm leading-7 text-slate-600">
              أدخل ISSN للحصول على أقوى مطابقة ممكنة. النظام لا يمنح Q
              اعتماداً على تشابه الاسم فقط، ويعرض مصدر الدليل وحدود التحقق
              بوضوح.
            </p>
          </div>
        ...
        <section className="rounded-[28px] border border-slate-200 bg-slate-950 p-5 text-white shadow-xl sm:p-7">
          <form
            onSubmit={verify}
            className="grid gap-4 lg:grid-cols-[.7fr_1.3fr_auto] lg:items-end"
          >
          ...
          <p className="mt-4 text-xs leading-6 text-slate-400">
            للأمان العلمي: المطابقة بالاسم وحده تستخدم للتعرف على البيانات فقط.
            اعتماد الربع يحتاج ISSN مطابقاً ومصدر بيانات مناسباً. إذا وصلت إلى
            هذه الصفحة من نتيجة بحث، تتم تعبئة الحقول تلقائياً وبدء التحقق.
          </p>
        </section>
            ...
            <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
              <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                ...
                <p className="mt-2 text-3xl font-black text-slate-950">
                  {result.metrics.quartile || "غير مؤكد"}
                </p>
                <p className="mt-2 text-xs leading-5 text-slate-500">
                  {result.metrics.quartile
                    ? "من SCImago Snapshot مطابق بـISSN"
                    : "لا يوجد دليل كافٍ لعرض Q"}
                </p>
              </div>
              ...
              <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                ...
                <p className="mt-2 text-lg font-black text-slate-950">
                  {statusLabel(result.indexing.scopus)}
                </p>
                <p className="mt-2 text-xs text-slate-500">
                  Source ID: {result.journal.sourceId || "—"}
                </p>
              </div>
              ...
              <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                ...
                <p className="mt-2 text-lg font-black text-slate-950">
                  {result.metrics.sjr || "—"} / {result.metrics.snip || "—"}
                </p>
                <p className="mt-2 text-xs text-slate-500">
                  H-index: {result.metrics.hIndex || "—"}
                </p>
              </div>
              ...
              <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                ...
                <p className="mt-2 text-xs leading-5 text-slate-500">
                  Clarivate: {clarivateLabel(result.indexing.clarivate)}
                </p>
              </div>
            </section>
                ...
                <div>
                  ...
                  <dl className="mt-5 space-y-3 text-sm">
                    <div className="flex justify-between gap-4 border-b border-slate-100 pb-3">
                      <dt className="font-bold text-slate-500">الناشر</dt>
                      <dd className="text-left font-black text-slate-900">
                        {result.journal.publisher || "—"}
                      </dd>
                    </div>
                    <div className="flex justify-between gap-4 border-b border-slate-100 pb-3">
                      <dt className="font-bold text-slate-500">ISSN</dt>
                      <dd className="font-black text-slate-900">
                        {result.journal.issn || "—"}
                      </dd>
                    </div>
                    <div className="flex justify-between gap-4 border-b border-slate-100 pb-3">
                      <dt className="font-bold text-slate-500">eISSN</dt>
                      <dd className="font-black text-slate-900">
                  ...
                  <div className="mt-3 space-y-3">
                    {result.evidence.length ? (
                      ...
                      <div className="rounded-2xl border border-dashed border-slate-300 bg-slate-50 p-4 text-sm text-slate-600">
                        لا يوجد دليل موثوق كافٍ لهذه المجلة حالياً.
                      </div>
                    )}
                  </div>
                </div>
            ...
            {["Q1", "Q2", "Q3", "Q4"].map((quartile) => (
              ...
              >
                ...
                <p className="mt-2 text-xs leading-6 text-slate-500">
                  يظهر فقط عندما توجد مطابقة آمنة للهوية ومصدر تصنيف واضح.
                </p>
              </div>
            ))}
          </section>
