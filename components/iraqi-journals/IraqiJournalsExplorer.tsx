...
export default function IraqiJournalsExplorer({
  ...
  const fields = useMemo(() => {
    const uniqueFields = Array.from(
      new Set(journals.map((journal) => journal.field).filter(Boolean))
    );

    return ["الكل", ...uniqueFields];
  }, [journals]);
  ...
  const filteredJournals = useMemo(() => {
    ...
    return journals.filter((journal) => {
      ...
      return matchesSearch && matchesStatus && matchesQuartile && matchesField;
    });
  }, [journals, searchTerm, statusFilter, quartileFilter, fieldFilter]);

  const classifiedCount = journals.filter(
    (journal) => journal.status === "مصنفة"
  ).length;

  const localCount = journals.filter(
    (journal) => journal.status === "محلية"
  ).length;

  const needsCheckCount = journals.filter(
      ...
      <section className="mx-auto max-w-7xl">
          ...
          <div className="mb-5 flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
            <div>
              <p className="text-sm font-bold text-slate-500">
                Iraqi Journal Search & Filters
              </p>
              <h2 className="text-2xl font-black text-slate-900">
                البحث والفلترة داخل المجلات العراقية
              </h2>
            </div>
          ...
          <div className="grid gap-4 lg:grid-cols-4">
            <div className="lg:col-span-4">
              <label className="mb-2 block text-sm font-bold text-slate-700">
                بحث باسم المجلة أو الجامعة أو التخصص
              </label>
              <input
                value={searchTerm}
                onChange={(event) => setSearchTerm(event.target.value)}
                placeholder="مثال: بغداد، قانون، بيئة، علوم، تربية..."
            ...
            <div>
              ...
              >
                ...
                <option value="مصنفة">مصنفة</option>
                <option value="محلية">محلية</option>
                <option value="تحتاج تحقق">تحتاج تحقق</option>
              </select>
            </div>
            ...
            <div>
              ...
              <select
                ...
                onChange={(event) => setQuartileFilter(event.target.value)}
                className="w-full rounded-2xl border border-slate-300 bg-white px-4 py-3 text-sm font-bold text-slate-800 outline-none focus:border-blue-500"
              >
                <option value="الكل">الكل</option>
            ...
            <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
              <p className="text-xs font-bold text-slate-500">نتائج مطابقة</p>
        ...
        <section className="mb-6 rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
          ...
          {filteredJournals.length === 0 ? (
            <div className="rounded-3xl border border-dashed border-slate-300 bg-slate-50 p-8 text-center text-sm font-bold text-slate-600">
              لا توجد مجلات مطابقة للفلاتر الحالية.
            </div>
          ) : (
